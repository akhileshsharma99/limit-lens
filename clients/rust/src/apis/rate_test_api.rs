...
/// struct for typed errors of method [`get_all_sessions`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetAllSessionsError {
    UnknownValue(serde_json::Value),
}
...
/// struct for typed errors of method [`metrics_dashboard`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum MetricsDashboardError {
    UnknownValue(serde_json::Value),
}
...
pub async fn get_all_sessions(configuration: &configuration::Configuration, ) -> Result<Vec<models::TestSession>, Error<GetAllSessionsError>> {

    let uri_str = format!("{}/api/test/sessions", configuration.base_path);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::TestSession&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::TestSession&gt;`")))),
        }
    } else {
        let content = resp.text().await?;
        let entity: Option<GetAllSessionsError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
...
pub async fn metrics_dashboard(configuration: &configuration::Configuration, ) -> Result<(), Error<MetricsDashboardError>> {

    let uri_str = format!("{}/dashboard", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MetricsDashboardError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
